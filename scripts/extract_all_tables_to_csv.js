...
function extractTableData(tableName) {
    ...
    const insertPattern = new RegExp(`INSERT INTO\\s+\`${tableName}\`\\s+\\(([^)]+)\\)`, 'i');
...
function findAllTables() {
    ...
    for (const line of lines) {
        ...
        if (match && match[1]) {
            ...
            if (tableName && !tableName.includes('_view') && tableName !== 'migrations') {
...
for (const tableName of allTables) {
    try {
        ...
        if (tableData && tableData.rows.length > 0) {
            ...
            console.log(`   ⚠️  Bảng ${tableName} không có dữ liệu hoặc không tìm thấy`);
            failCount++;
        }
    } catch (error) {
